        ...
        for (Node childNode = node.getFirstChild();
            ...
            if (childNode.getNodeType() == Node.ELEMENT_NODE) {
                if (childNode.getNodeName().equals("car")) {
                    final Document doc = childNode.getOwnerDocument();
                    final Element company = doc.createElement("company");
                    final Text text = doc.createTextNode("カーバンク社");
                    company.appendChild(text);
                    childNode.appendChild(company);
                }
                walk(childNode);
            }
        }
